...
| Pre-compaction memory flush | `precompact-flush.py` deterministic raw-dump (v3.5.1+: NEVER blocks `/compact`) |
| Auto-detect embedding provider | `_embed.py` auto-detect from env |

Key gaps identified vs OpenClaw (staged consolidation, multi-signal ranking, contradiction
detection) were closed in v2.9–v3.2. See `.claude/research/product-architecture.md` for full comparison.
...
```bash
python3 -m unittest discover -s tests                 # full suite (517 tests)
...
- **v4.1.1 / v4.1.2 (post-tag patches)**: the two untested-path failures behind the pre-tag smoke-test rule above — `_setup.py` skills backup leaked `.git`/VCS internals into the synced vault (v4.1.1: excluded, plus survive read-only leftovers); `fix_aspect` derived slugs now clamped to 60 chars and `route()` never trusts frontmatter slugs (v4.1.2: unclamped slug crashed routing on first live use). Suite was **402 tests** at that point.
- **v4.3 (Zero-Mem retrieval repair)**: grounded in arXiv 2607.29377v1 ("Zero-Mem: Zero-Token Memory Operations for LLM Agents"). The audit found the deterministic machinery already here was **not working** — every number measured on the live 15,145-chunk vault. (A) **`_profile.py`** adopts eq (6) `phi(q)` + a safe FTS5 builder: bare multi-word text is implicit-AND, so **every natural-language query returned 0 hits** (`why did we drop vector recall` → 0; OR-joined → 991), and `vector-recall`/`forget: ttl` raised `OperationalError` swallowed into a silent "(no results)". `query_ex()` now reports WHY a set is empty; `query_by_type` keeps its `list[dict]` contract. (B) **tag column resurrected 54 → 2,528 chunks** (0.4% → 16.7%): `split_chunks` lifts `## [decision] Title` into `heading` but `_extract_tag` probed only `content` (4,714 chunks); frontmatter was chunked as content so every routed write landed untagged (635); `- [exp] …` was unrecognised. Self-healing backfill, 0.43s on the real index. (C) **heading indexed** as a 4th `chunks_fts` column (weight 4.0; `bm25()` weights derived from actual arity) — a term appearing only in an entry title was previously unfindable. (D) **workspace filter moved into SQL** — it ran after `LIMIT`, so `--ws personal --limit 20 python` returned nothing while 3 chunks matched. (E) **`reindex_paths()`** wired into `append_entry`/`append_lesson` (index was 5 days stale; `_index_one`/`_drop_path_rows` factored out, FTS rowids deleted before content rows). (F) **bootstrap fixed**: the first file got the whole budget, so `handoff.md` was dropped from **every session** (2/5 files, ~124,500 tokens/day buying less recall than designed) — budget now reserved for per-session deltas before statics are read, emission order unchanged for prompt-cache stability; 2/5 → 5/5. (G) **per-path collapse** (eq 14 Dedup). (H) 6 commands + 7 skills repointed off the dead pre-v2.7 layout — `/mem-journal` was writing memory to `$PWD/docs/journal/` (unsynced, unindexed) and `/mem-cost` measured 0 of 9 files, which is *why* (F) survived; it now calls `bootstrap-load.py --report`, sharing `_plan()` with the hook. **REJECTED the paper's entity graph + PPR** (its largest ablation delta): a working stdlib prototype LOWERED MRR at every rho (1.000 vs 0.900 vs 0.767) on 5 real queries. +61 tests (**463 total**). Spec: `docs/superpowers/specs/2026-08-05-zero-mem-retrieval-design.md`.
- **v4.7 (delegation-first cadence blocks)**: the every-N-turn Stop-hook work no longer runs in the MAIN context (it cost 10-20 unrelated tool calls per cadence, diluting the user's flow — user-reported). Journal block → dispatch ONE background **memory teammate** whose turn source is the `_capture.py` session log (inline fallback when no log/subagent tool; anti-recursion sentinel in the prompt); review block → dispatch a fresh-context **judge** (must NOT be a context-inheriting fork), main relays its 3-line summary; no log → review DEFERRED (counter kept) + once-per-session `[gowth-mem:review-paused]` notice naming `reflection.capture_enabled` (prefix deliberately ≠ the live `[gowth-mem:self-review` trigger token external skills key on). Capture gated by `reflection.capture_enabled` defaulting to `reflection.enabled` — the documented privacy opt-out survives (session logs sync to the git remote). Collision turn → `[gowth-mem:both-cadences]` TWO-SEPARATE-subagents header keyed off what fired. Templates: SKIP guards against self-dispatch, teammate Anchors (vault root from session-log path, `GOWTH_MEM_HOME`-safe), dead `{ws_list_str}`/`{prune_summary}` placeholders removed. `_run_forget()` also fires on the review cadence when journal cadence is off (was: captured forever, never archived), guarded against per-Stop spawns at `turn_interval: 1`. Shipped through 3 adversarial fresh-context review rounds (12+4+2 findings, all fixed → APPROVE); every changed path exercised live pre-tag. +54 tests (**517 total**).
- **v4.7.1 (delegation hardening)**: closes the 15 verified findings of a max-effort adversarial audit of the v4.7 range. **Privacy sealed**: the settings example no longer ships `capture_enabled: true` (it defeated the `reflection.enabled: false` opt-out on every `/mem-install`-scaffolded vault — the default chain now governs, pinned by test); `_coerce_bool` on hand-edited knobs (JSON string `"false"` no longer truthy, explicit `null` = default chain); capture runs even with BOTH cadences off (/mem-review-only users). **Hook can't traceback**: `session_id` str-coerced (a JSON number crashed the `[:8]` slice every Stop, rc=1 — live-reproduced) + `__main__` guard → entrypoint ALWAYS exits 0. **TTL archival cadence-independent**: `_run_forget_daily()` at most once/day via state.json `forget_last_run` (was: per-Stop spawns at `turn_interval: 1`, never with both cadences off). **Review integrity**: pre-dispatch signal floor `reflection.min_review_turns` (default 10 = rubric §0b — no more guaranteed-no-op judge dispatches), paused notice persisted once-per-session (`review_paused_notified`) + carries the `/mem-review-backlog` nudge, `_reset_counters` unlocked fallback on lock timeout (was: duplicate judge), judge appends its review block via the locked `_capture.py --append-review` CLI (a raw Write raced capture's read-modify-write → silently lost turns or the whole review), midnight date-split passes BOTH logs and the floor counts across them. **Dispatch contract pinned** (`tests/test_dispatch_contract.py`): teammate template mirrors the hook reason verbatim (sentinel + absolute paths), judge SKIP guard keys on payload not identity, judge rubric gains 0c Anchors (never resolve `<ws>` against cwd — the v4.3 `$PWD` bug class) + Backlog JSONL mode, teammate template names the write interface (`_topic.py --append`, `GOWTH_MEM_HOME`-prefixed `_moc.py`). Docs truthful: `/mem-review` names `capture_enabled`/`min_review_turns`, drops the false "actions trace is always captured". +32 tests (**549 total**).

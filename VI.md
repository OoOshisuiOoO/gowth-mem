...
v2.0 trả lời ba vấn đề: 1 thư mục `~/.gowth-mem/`, sắp xếp theo topic, an toàn khi chạy song song, tự động pull/push quanh `/compact`, AI giúp resolve conflict. Từ v2.7, layout chia thành `shared/` (cross-workspace) + `workspaces/<ws>/` (per-workspace).

## Kiến trúc

```
~/.gowth-mem/                         1 chỗ duy nhất, dùng chung mọi project
├── shared/                           ★ kiến thức cross-workspace
│   ├── AGENTS.md                     global operating rules (sync)
│   ├── secrets.md                    POINTER (env-var name; KHÔNG bao giờ value)
│   ├── tools.md                      tool quirks dùng chung
│   └── skills/<slug>.md              workflow tái dùng kiểu Voyager
├── workspaces/<ws>/                  ★ kiến thức per-workspace
│   ├── docs/
│   │   ├── handoff.md                session state (prefix host:<máy>)
│   │   ├── exp.md                    kinh nghiệm debug/fix
│   │   ├── ref.md                    fact đã verify
│   │   ├── tools.md                  tool quirks theo workspace
│   │   └── files.md                  cấu trúc project
│   ├── journal/<date>.md             log thô hằng ngày
│   ├── skills/<slug>.md              workflow riêng workspace
│   └── <slug>/<slug>.md              topic file (1 folder / topic)
...
Helper `_topic.py` quyết định 1 entry vào file nào trong active workspace:

1. Trích keywords ≥4 ký tự (bỏ stopword).
2. Đếm overlap với từng `workspaces/<ws>/<slug>/<slug>.md` đang có.
...
Trong topic file dùng `[[other-slug]]` (cùng workspace), `[[ws:slug]]` (cross-workspace), hoặc `[[shared:secrets]]` (shared registries). Hook recall sẽ follow 1 hop khi top hit là 1 topic file.
...
1. Tạo `~/.gowth-mem/shared/` + workspace mặc định via `_workspace.py`.
2. Copy templates: `shared/AGENTS.md`, `settings.json`, `shared/{secrets,tools}.md`.
3. Hỏi bạn 3 câu:
...
- `docs/{exp,ref,tools}.md` lines → routed vào `~/.gowth-mem/workspaces/<ws>/<slug>/<slug>.md` (Source: ws/file).
...
3. **Không có**: grep workspace topic files và `docs/*.md` (skip `journal/`).
4. Skip dòng có `(superseded)` hoặc `valid_until:` đã hết hạn.
5. Tier-score: `journal/today (100)` > `workspace topics (80)` > `journal/yesterday (70)` > `docs/* (60)` > `skills/ (40)` > khác.
...
Hook `user-augment.py` luôn inject content `~/.gowth-mem/shared/AGENTS.md` dạng `<Rules>...</Rules>` ở đầu mỗi UserPrompt. Lý do:
...
Cho long-term, project-bound knowledge: `/save` (claude-obsidian) → `wiki/concepts/`. Cho cross-project: ở `~/.gowth-mem/workspaces/<ws>/` hoặc `shared/`.

...
def _archive_one(f: Path, ws: str, gh: Path, dry_run: bool) -> bool:
    """Gzip `f` into .archive/journal/<ws>/ then remove the original."""
    try:
...
def forget_workspace(ws: str, ttl_days: int, max_bytes: int, salvage: bool,
    ...
    for f in sorted(jd.glob("*.md")):
        if f.name == SALVAGE_FILE or f.name == today_name:
            continue
        try:
            st = f.stat()
        except OSError:
        ...
        too_big = st.st_size > max_bytes and st.st_mtime < (time.time() - 86400)
    ...
    try:
        with file_lock(f"journal-{ws}", timeout=5.0):
            if salvage:
                salvaged = _salvage_entries(candidates, jd, dry_run)
            for f in candidates:

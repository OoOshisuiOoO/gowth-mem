...
Migrate v1.0 per-workspace memory into the current `shared/` + `workspaces/` layout.
...
2. **For each source workspace** `<ws>`:
   - Create a target workspace via `_workspace.py create <basename(ws)>` if it doesn't exist.

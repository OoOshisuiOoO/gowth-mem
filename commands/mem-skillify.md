---
description: Extract a recurring workflow from the current session into a reusable skill file in the vault at workspaces/<ws>/skills/<name>.md (Voyager pattern). Future sessions invoke the skill by short reference instead of re-deriving the steps.
...
4. Write `~/.gowth-mem/workspaces/<ws>/skills/<name>.md` with sections: Description / Steps / Inputs / Variations / Source.
5. Update `~/.gowth-mem/workspaces/<ws>/docs/files.md` to reference the new skill (so it's discoverable).
